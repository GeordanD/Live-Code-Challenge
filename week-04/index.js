...
function organizedMovies() {
    for (let i = 0; i < movies.length; i++) {
        // console.log(movies[i].ageRating);
        if (movies[i].ageRating === 'PG-13') {
            pg13Movies.push(movies[i]);
        } else if (movies[i].ageRating === 'PG-13' || movies[i].ageRating === "G") {
            console.log(movies[i].ageRating);
...
const checkGenre = () => {
    for (let i = 0; i < movies.length; i++){
        ...
        if (
          movies[i].genre.includes("comedy") && movies[i].genre.includes("family") ) {
...
let movieOne = new Movie("Brave Little toaster", ['animation', 'adventure', 'g'])
